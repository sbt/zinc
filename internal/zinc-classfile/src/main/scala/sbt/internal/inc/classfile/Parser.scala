...
private[sbt] object Parser:
    ...
    new ClassFile:
      ...
      private def parseInnerClasses(a: AttributeInfo): Array[InnerClassInfo] =
        ...
        Array.tabulate(numberOfClasses) { _ =>
          ...
          val innerName = if innerNameIndex == 0 then None else Some(toUTF8(innerNameIndex))
          val innerCN =
            if innerClassInfoIndex == 0 then ""
            else getClassConstantName(innerClassInfoIndex)
          val outerCN =
            if outerClassInfoIndex == 0 then ""
      ...
      private def parseAttribute() =
        ...
        val name = if nameIndex == -1 then None else Some(toUTF8(nameIndex))
      ...
      def types: Set[String] =
        ...
        (classConstantReferences ++ fieldTypes ++ methodTypes ++
          annotationsReferencesCarefully).toSet
      ...
      private def attributeReferences(attr: AttributeInfo): List[String] =
        if attr.isRuntimeVisibleAnnotations || attr.isRuntimeInvisibleAnnotations then
        ...
        else if attr.isRuntimeVisibleTypeAnnotations || attr.isRuntimeInvisibleTypeAnnotations then
          parseTypeAnnotations(attr.value) // JVMS 4.7.20
        else if attr.isAnnotationDefault then
      ...
      private def parseParameterAnnotations(value: Array[Byte]): List[String] =
        withAnnotationStream(value) { (in, result) =>
          ...
          for _ <- 0 until numParameters do
      ...
      private def skipTypeAnnotationTarget(in: DataInputStream): Unit =
        ...
        for _ <- 0 until in.readUnsignedByte() do // type_path: u1 path_length, then path entries
          in.readUnsignedByte(); in.readUnsignedByte()
      ...
      private def nestedAttributes(code: AttributeInfo): Array[AttributeInfo] =
        ...
        for _ <- 0 until in.readUnsignedShort() do skipFully(in, 8)
      ...
      private def readNestedAttributes(in: DataInputStream): Array[AttributeInfo] =
        ...
        for _ <- 0 until count do
          ...
          if isAnnotationAttributeName(name) then
      ...
      private def constants =
        ...
        def next(i: Int, list: List[Constant]): List[Constant] =
          if i < constantPool.length then
            val constant = constantPool(i)
            next(if constant.wide then i + 2 else i + 1, constant :: list)
          else
            list
        next(1, Nil)
    end new
  ...
  private def parseConstantPool(in: DataInputStream) =
    ...
    def parse(i: Int): Unit =
      if i < constantPoolSize then
        val constant = getConstant(in)
        pool(i) = constant
        parse(if constant.wide then i + 2 else i + 1)
  ...
  private def toInt(v: Byte) = if v < 0 then v + 256 else v.toInt
