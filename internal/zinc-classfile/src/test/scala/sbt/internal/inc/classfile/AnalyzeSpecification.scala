...
class AnalyzeSpecification extends UnitSpec:
  ...
  // binary deps.
  "Analyze" should
    "record transitive ancestry of a referenced classpath type as binary deps (sbt/zinc#148)" in {
      IO.withTemporaryDirectory { temp =>
        val classesDir = new File(temp, "classes")
        classesDir.mkdir()
  ...
  "Analyze" should
    "track a javax.* ancestor served from the classpath, not the JDK (sbt/zinc#148)" in {
      IO.withTemporaryDirectory { temp =>
        val classesDir = new File(temp, "classes")
        classesDir.mkdir()
