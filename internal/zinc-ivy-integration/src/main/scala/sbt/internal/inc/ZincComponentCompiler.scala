  ...
  private[sbt] def getDefaultBridgeModule(scalaVersion: String): ModuleID = {
    def compilerBridgeId(scalaVersion: String) = {
      scalaVersion match {
        ...
        case sc if (sc startsWith "2.13.0-pre-") => "compiler-bridge_2.13.0-M4"
        case sc if (sc startsWith "2.13.0-M")    => "compiler-bridge_2.13.0-M4"
        case sc if (sc startsWith "2.13.0-RC")   => "compiler-bridge_2.13.0-M4"
