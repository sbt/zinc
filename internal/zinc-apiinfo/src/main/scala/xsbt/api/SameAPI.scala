...
object SameAPI:
  ...
  def filterDefinitions(ds: Seq[Definition], topLevel: Boolean, includePrivate: Boolean) =
    if topLevel || includePrivate then ds else ds.filter(APIUtil.isNonPrivate)
end SameAPI
...
class SameAPI(includePrivate: Boolean, includeParamNames: Boolean):
  ...
  def debug(flag: Boolean, msg: => String): Boolean =
    if debugEnabled && !flag then println(msg)
    flag
  ...
  def sameDefinitions(
    ...
    debug(
      ...
      "\tDefinition strings differed (a: " + (a.keySet.toSet -- b.keySet) + ", b: " +
        (b.keySet
          .toSet --
          a
            .keySet) + ")"
          ...
          def sameDef(seen: List[Definition], remaining: List[Definition]): Boolean =
            remaining match
              ...
              case bdef :: btail =>
                ...
                if eq then sameDefs(atail, seen ::: btail) else sameDef(bdef :: seen, btail)
          sameDef(Nil, b)
        case Nil => true
    debug(
      a.length == b.length,
  ...
  def bitSet(m: Modifiers): immutable.BitSet =
    ...
    (modifiers foldLeft immutable.BitSet.empty) { case (bs, (mod, i)) =>
      if mod then bs + i else bs
    }
  ...
  def sameFieldSpecificAPI(a: FieldLike, b: FieldLike): Boolean =
    debug(
      ...
      "Different field categories (" + a.name + "=" + a.getClass.getName + " -- " + a.name + "=" +
        a
          .getClass.getName + ")"
    ) &&
  ...
  def sameClassLikeSpecificAPI(a: ClassLike, b: ClassLike): Boolean =
    debug(sameTopLevel(a, b), "Top level flag differs") &&
      ...
      sameSeq(
        a.childrenOfSealedClass.toIndexedSeq,
        b.childrenOfSealedClass.toIndexedSeq
      )(sameType) &&
  ...
  private def samePending[T](a: T, b: T)(f: (T, T) => Boolean): Boolean =
    if pending add ((a, b)) then f(a, b) else true
  ...
  def differentCategory(label: String, a: AnyRef, b: AnyRef): Boolean =
    debug(
      flag = false,
      "Different category of " + label + " (" + a.getClass.getName + " and " +
        b.getClass
          .getName + ") for (" + a + " and " + b + ")"
    )
