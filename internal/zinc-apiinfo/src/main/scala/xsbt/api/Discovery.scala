...
import xsbti.api.{ Path as APath, * }
...
class Discovery(baseClasses: Set[String], annotations: Set[String]):
  ...
  def apply(d: Definition): Discovered =
    d match
      case c: ClassLike if isConcrete(c.modifiers) =>
        if isPublic(c) then
          discover(c)
        else if isModule(c) && hasMainMethod(c)
        then // jvm does not require a main class to be public
  ...
  def bases(
      own: String,
      c: Seq[Type]
  ): Set[String] = (own +: c.flatMap(simpleName)).filter(baseClasses).toSet
end Discovery
...
object Discovery:
  ...
  def isParameterized(t: Type, base: String, args: String*): Boolean = t match
    case p: Parameterized =>
      named(p.baseType, base) && p.typeArguments.length == args.length &&
      p.typeArguments
        .flatMap(simpleName)
        .sameElements(args)
    case _ => false
