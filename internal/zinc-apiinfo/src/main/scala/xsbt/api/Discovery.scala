...
class Discovery(baseClasses: Set[String], annotations: Set[String]) {
  def apply(s: Seq[Definition]): Seq[(Definition, Discovered)] = s.map(d => (d, apply(d)))

  def apply(d: Definition): Discovered = d match {
...
object Discovery {
  ...
  def findAnnotations(as: Seq[Annotation], pred: String => Boolean): Set[String] = as.flatMap { a =>
  ...
  def defAnnotations(defs: Seq[Definition], pred: String => Boolean): Set[String] = findAnnotations(
    defs.flatMap {
  ...
  def hasMainMethod(defs: Seq[Definition]): Boolean = defs.exists(isMainMethod)
  ...
  def isStringArray(t: Type): Boolean =
    ...
    isParameterized(t, "scala.Array", "java.lang.String")
