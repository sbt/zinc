...
object APIUtil {
  ...
  def minimize(api: ClassLike): ClassLike = minimizeClass(api)
  def minimizeDefinitions(ds: Array[Definition]): Array[Definition] = ds.flatMap(minimizeDefinition)

  def minimizeDefinition(d: Definition): Array[Definition] = d match {
  ...
  def minimizeStructure(s: Structure, isModule: Boolean): Structure = Structure.of(
    lzy(s.parents),
  ...
  def isNonPrivate(access: Access): Boolean = access match {
