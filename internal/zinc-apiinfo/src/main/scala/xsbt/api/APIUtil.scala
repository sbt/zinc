...
object APIUtil:
  val modifiersToByte = (m: Modifiers) =>
    import m.*
    def x(b: Boolean, bit: Int) = if b then 1 << bit else 0
  ...
  def filterDefinitions(
    ...
    val mains = if isModule then ds.filter(Discovery.isMainMethod) else emptyClassDefs
    if mains.isEmpty then emptyClassDefsLzy else lzy(mains)
