...
trait FormatCommons:
  ...
  protected def expectHeader(in: BufferedReader, expectedHeader: String): Unit =
    ...
    if header != expectedHeader + ":" then
      throw new ReadException(expectedHeader, if header == null then "EOF" else header)
  ...
  protected def writeSeq[T](out: Writer)(header: String, s: Seq[T], t2s: T => String): Unit =
    ...
    def n = s.length
    val numDigits = if n < 2 then 1 else math.log10(n.toDouble - 1).toInt + 1
