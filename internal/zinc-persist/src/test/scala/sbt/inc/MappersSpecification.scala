...
class MappersSpecification extends Properties("TextAnalysisFormat") {
  ...
  property("Maps UsedName correctly") = forAll(TestCaseGenerators.genUsedName())(checkName)
  ...
  property("Maps EnumSet correctly") = {
    ...
    def check(useScopes: util.EnumSet[UseScope]) =
      useScopes =? serizlizer.deserialize(serizlizer.serialize(useScopes))
