...
import xsbti.api.{ SafeLazyProxy, _ }
...
class TestAnalysisCallback(
  ...
  val classPublicNameHashes = new HashMap[String, Array[NameHash]]
  val objectPublicNameHashes = new HashMap[String, Array[NameHash]]
  ...
  def usedName(className: String, name: String, scopes: util.EnumSet[UseScope]): Unit = { usedNames(className) += name }
