...
private final class MSourceInfos(val allInfos: Map[VirtualFileRef, SourceInfo])
  ...
  def ++(o: SourceInfos) =
    ...
    (allInfos.keySet ++ o.allInfos.keySet).foreach { file =>
      ...
      val res =
        if lhs == null then rhs else if rhs == null then lhs else SourceInfos.merge1(lhs, rhs)
  ...
  def add(file: VirtualFileRef, info: SourceInfo) =
    ...
    val infos =
      allInfos.updated(file, if info0 == null then info else SourceInfos.merge1(info0, info))
    new MSourceInfos(infos)
