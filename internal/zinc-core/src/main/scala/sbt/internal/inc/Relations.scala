...
private class MRelationsNameHashing(
  ...
  private[inc] def addUsedNames(data: Relations.UsedNames): Relations =
    new MRelationsNameHashing(
      ...
      externalDependencies,
      classes,
      names = if names.isEmpty then data else names ++ data,
      productClassName,
    )
  ...
  override def toString: String =
    def deps_s(m: Map[?, Relation[?, ?]]) =
      val rendered = m.iterator.toVector.sortBy(_._1.toString).map { case (kind, relation) =>
        ...
        if values == "none" then s"\n    $kind: none" else s"\n    $kind:$values"
      }
      if rendered.isEmpty then "none" else rendered.mkString
    def field(name: String, value: String): String =
      if value == "none" then s"$name: none" else s"$name:$value"
