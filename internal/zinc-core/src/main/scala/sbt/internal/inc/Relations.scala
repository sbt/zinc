...
object Relations {
  ...
  ): ClassDependencies = new ClassDependencies(internal, external)
...
private case class InternalDependencies(
  ...
  def +(dep: InternalDependency): InternalDependencies = InternalDependencies(
...
private case class ExternalDependencies(
  ...
  /** Adds `dep` to the dependencies */
  def +(dep: ExternalDependency): ExternalDependencies = ExternalDependencies(
    dependencies.updated(
      dep.context,
      dependencies
  ...
  /** Removes all dependencies from `sources` to another file from the dependencies */
  def --(classNames: Iterable[String]): ExternalDependencies = ExternalDependencies(
    dependencies.iterator
...
private class MRelationsNameHashing(
  ...
  def internalClassDeps(className: String): Set[String] = internalClassDep.forward(className)
  def usesInternalClass(className: String): Set[String] = internalClassDep.reverse(className)
  ...
  override def localInheritance: ClassDependencies = new ClassDependencies(
  ...
  override def memberRef: ClassDependencies = new ClassDependencies(
  ...
  override def toString: String = {
    def deps_s(m: Map[_, Relation[_, _]]) = m.iterator.map { case (k, vs) =>
      s"\n    $k ${relation_s(vs)}"
    }.mkString
    s"""
    |Relations:
